...
class AgentCore:
    ...
    def _call_third_party_chat(self, text: str, user_id: str) -> str:
        """
        调用第三方陪聊服务。
        优先使用通义千问(Qwen)，未配置时返回兜底回复。
        """
        # 1. 如果配置了自定义fetcher，优先使用
        if self._third_party_fetcher:
            return self._third_party_fetcher(text, user_id)
        
        # 2. 调用通义千问
        dashscope_key = getattr(self, '_dashscope_api_key', None)
        if dashscope_key:
            try:
                return self._call_qwen(text, user_id, dashscope_key)
            except Exception as e:
                logger.error("通义千问调用失败: %s", e)
                # 失败时返回本地兜底回复
                return self._local_chat_reply(text)
        
        # 3. 未配置任何服务，返回兜底回复
        return self._local_chat_reply(text)

    def _call_qwen(self, text: str, user_id: str, api_key: str) -> str:
        """调用阿里云DashScope通义千问API"""
        import urllib.request
        import json
        
        url = "https://dashscope.aliyuncs.com/api/v1/services/aigc/text-generation/generation"
        
        # 构建适合老年人的system prompt
        system_prompt = """你是"小守"，一位专门为老年人服务的智能助手。
你的特点：
- 说话温和、耐心、简单易懂
- 避免使用网络流行语和复杂术语
- 回答简洁，一次只说一件事
- 如果老人提到身体不适或紧急情况，要优先提醒联系家人或医生
- 可以适当关心老人的日常起居，但不要过度追问隐私

当前用户是 elderly_user，请用适合老年人的方式回答。"""
        
        payload = {
            "model": "qwen-turbo",  # 使用轻量版，速度快成本低
            "input": {
                "messages": [
        ...
        if "output" in result and "choices" in result["output"]:
            ...
            if choices and len(choices) > 0:
                ...
                return message.get("content", "抱歉，我没听清楚，您能再说一遍吗？")
        
        return "抱歉，我现在有点忙，稍后再和您聊好吗？"

    def _local_chat_reply(self, text: str) -> str:
        """本地兜底回复，当第三方服务不可用或热词匹配失败时调用。"""
        # 简单关键词匹配做基础回复
        text_lower = text.lower()
        
        greetings = ["你好", "您好", "早上好", "下午好", "晚上好", "小守"]
        for g in greetings:
            if g in text:
                return "您好呀！我是小守，今天过得怎么样？有什么我可以帮您的吗？"
        
        if "谢谢" in text or "感谢" in text:
            return "不客气！能帮到您我也很开心。有什么需要随时叫我。"
        
        if "再见" in text or "拜拜" in text:
            return "好的，您先忙，有需要随时找我聊天。保重身体！"
        
        if "名字" in text or "叫什么" in text:
            return "我叫小守，是您的智能助手。我可以陪您聊天，也能帮您查天气、记健康、紧急求助。"
        
        if "无聊" in text or "没事做" in text or "闷" in text:
            return "我陪您说说话吧。您今天有没有出去走走？天气好的话，散散步对心情和身体都好。"
        
        if "睡不着" in text or "失眠" in text:
            return "失眠确实难受。可以试试睡前泡泡脚，或者听点轻音乐。如果经常这样，建议您跟子女或医生说说。"
        
        if "烦" in text or "生气" in text or "郁闷" in text:
            return "别生气，气坏了身体不值得。跟我说说，是什么让您不开心了？说出来可能会好受些。"
        
        # 兜底回复
        return (
            f"您说：{text[:50]}{'...' if len(text) > 50 else ''}\n\n"
            "我在认真听。您是想聊聊天，还是需要我帮您查天气、记录健康情况呢？"
        )
